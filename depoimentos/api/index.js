...
app.post('/redefinir-senha', async (req, res) => {
  ...
  try {
    ...
    const salt = await bcrypt.genSalt(10);
    const senhaHash = await bcrypt.hash(senha, salt);
    await pool.query('UPDATE usuarios SET senha_hash = $1, reset_token = NULL, reset_token_expires = NULL WHERE id = $2', [senhaHash, user.id]);
...
app.get('/perfil', autenticarToken, async (req, res) => {
  try {
    ...
    const perfilResult = await pool.query('SELECT id, nome, email, foto_perfil_url, bio, versiculo_favorito, cidade FROM usuarios WHERE id = $1', [userId]);
...
app.put('/perfil', autenticarToken, async (req, res) => {
  try {
    ...
    if (versiculo_favorito !== undefined) { fields.push(`versiculo_favorito = $${queryIndex++}`); values.push(versiculo_favorito); }
    if (cidade !== undefined) { fields.push(`cidade = $${queryIndex++}`); values.push(cidade); }
    if (fields.length === 0) { return res.status(400).json({ error: 'Nenhum campo para atualizar foi fornecido.' }); }
    values.push(userId); 
    const query = `UPDATE usuarios SET ${fields.join(', ')} WHERE id = $${queryIndex} RETURNING id, nome, email, foto_perfil_url, bio, versiculo_favorito, cidade;`;
