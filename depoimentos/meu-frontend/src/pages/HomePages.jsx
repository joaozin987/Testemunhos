// 1. CORREÇÃO: Importamos todas as ferramentas necessárias
...
function HomePage() {
  ...
  // 2. CORREÇÃO: Organizamos toda a lógica inicial dentro de um único useEffect
  useEffect(() => {
    // A proteção de login está desativada para podermos desenvolver a página
    ...
    // A função para carregar os depoimentos fica DENTRO do useEffect
    ...
    // Descomente a linha abaixo quando sua API estiver rodando
    // carregarDepoimentos(); 
    
    // Apenas para teste, para não mostrar "carregando" para sempre
    setIsLoading(false);

  }, []); // Deixamos o array de dependências vazio para rodar só uma vez
  ...
  return (
    <>
      ...
      <div className="bg-accent200">
        <section className="hero px-4 max-w-4xl mx-auto">
          ...
          {isWordSectionVisible && (
            <div className="flex flex-col items-center w-full mx-auto mt-6 p-4 rounded shadow bg-accent100">
              ...
              />
              <button className="p-2 bg-gray-100 text-xl font-slab rounded-lg w-full mt-3">Buscar a Palavra</button>
              <div className="verse-of-day mt-4 text-white text-center"></div>
            </div>
          )}
        </section>
        ...
        {/* ... Seção para listar os depoimentos da API ... */}
        ...
        {/* Adicione os outros modais aqui com a mesma lógica... */}
